...
Now that we have established our priors, our modeling procedure is straightforward. We derive the full conditionals for $\mathbf \theta$ and $\mathbf\Sigma$ and find the following expressions for them:
$$
\begin{equation}
...
We can then run a Gibbs sampler to find our posterior samples.
...
Sigma<-cov(NA_data) ; THETA<-SIGMA<-NULL #setting initial parameters for Gibbs
YS<-NULL
set.seed(1)
...
```{r trace}
library(coda)
plot(seq(10000), THETA[,2], type = "l")
acf(THETA,lag.max=50)
effectiveSize(THETA)
```
